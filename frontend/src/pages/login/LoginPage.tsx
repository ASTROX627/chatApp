...
const LoginPage: FC = () => {
  ...
  return (
    <div className="bg-gray-400/10 backdrop-blur-lg p-6 flex flex-col min-w-48 md:min-w-96 items-center justify-center rounded-lg shadow-lg">
      <h1 className="text-white text-xl md:text-2xl lg:text-3xl font-semibold mb-1 md:mb-3">{t("auth.login")} <span className={twMerge(classes.primary.text)}>{t("auth.chatApp")}</span></h1>
      <LoginFrom />
    </div>
  )
}
